      ...
      {!audioUrl ? (
        <div className="grid grid-cols-2 gap-4">
          <div
            ...
            className="border-2 border-dashed border-gray-300 rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer hover:border-emerald-500/50 hover:bg-gray-50 transition-all group"
          >
            {isUploading ? (
              <Loader2 className="w-6 h-6 text-emerald-500 animate-spin mb-2" />
            ) : (
              <Upload className="w-6 h-6 text-gray-500 group-hover:text-emerald-500 mb-2 transition-colors" />
            )}
            <p className="text-sm text-gray-600 group-hover:text-gray-700 text-center">
          ...
          <div
            ...
            className="border-2 border-dashed border-gray-300 rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer hover:border-indigo-500/50 hover:bg-gray-50 transition-all group"
          >
            {isCloning ? (
              <Loader2 className="w-6 h-6 text-indigo-500 animate-spin mb-2" />
            ) : (
              <Mic className="w-6 h-6 text-gray-500 group-hover:text-indigo-500 mb-2 transition-colors" />
            )}
            <p className="text-sm text-gray-600 group-hover:text-gray-700 text-center">
        ...
        <div className="w-full bg-gray-100 rounded-xl p-4 border border-gray-300 flex items-center gap-3">
          <Music className="w-5 h-5 text-emerald-600" />
          <div className="flex-1 overflow-hidden">
            <p className="text-xs text-gray-600 truncate">{audioUrl}</p>
          </div>
      ...
      {showTestAudio && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 rounded-2xl border border-slate-700 p-8 max-w-md w-full space-y-6">
            ...
            <div className="bg-slate-800 rounded-lg p-4">
              <p className="text-xs text-slate-400 leading-relaxed">
                <strong className="text-slate-300">💡 Next Steps:</strong> You can now use this voice ID to generate speech in any language or accent. The voice model has captured your unique vocal characteristics.
              </p>
            </div>
