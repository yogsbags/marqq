        ...
        >
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              {model.badge && (
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-purple-100 text-purple-700 uppercase tracking-wider font-semibold">
                  {model.badge}
                </span>
              )}
            </div>
            <div className="text-xs text-gray-600">{model.features}</div>
          </div>
          <div className="text-xs font-mono text-gray-500">{model.cost}</div>
        </button>
      ))}
    </div>
  );
