...
export function EnhancedBulkGenerator() {
  ...
  const approveAllForStage = async (
    ...
    if (result.approved > 0) {
      ...
      addLog("ℹ️  All items already approved");
    }
  };
  ...
  const executeStage = async (stageId: number) => {
    ...
    try {
      ...
      try {
        ...
        addLog(
          `⚠️  Warning: Could not auto-approve: ${
                        ...
                        <div className="max-h-96 overflow-y-auto">
                          {stageData[stage.id].data.map(
                            (row: any, idx: number) => (
                              <div
                                key={idx}
                                className="mb-3 p-3 bg-gray-50 dark:bg-gray-700/50 rounded border border-gray-200 dark:border-gray-600 text-xs relative"
                              >
                                {/* Edit Button */}
                                <button
                                  onClick={() =>
          ...
          <div className="bg-gray-900 dark:bg-gray-950 rounded-lg p-4 h-96 overflow-y-auto font-mono text-sm">
            {logs.length === 0 ? (
              ...
              logs.map((log, index) => (
                <div
                  key={index}
                  className="text-green-400 dark:text-green-500 mb-1"
                >
                  {log}
                </div>
              ))
