...
class WorkflowOrchestrator {
  constructor(config = {}) {
    ...
    this.seoDataFetcher = new SEODataFetcher({
      ...
      propertyId:
        config.propertyId || process.env.GA4_PROPERTY_ID || "309159799",
      country: "in",
      language: "en",
    });
  ...
  async executeFullWorkflow(options = {}) {
    ...
    console.log(
      `🤖 Auto-approve: ${this.config.autoApprove ? "Enabled" : "Disabled"}`,
    );
  ...
  async executeStage1Research(options = {}) {
    ...
    try {
      console.log(
        `🎯 Generating ${this.config.gapsPerRun} content gap opportunities...`,
      );

      // Use customTopic if provided, otherwise use category
      const category =
        options.category || this.config.category || "derivatives";
      ...
      console.log(
        `📝 ${researchData.content_gaps.length} gaps saved to: data/research-gaps.csv`,
      );

      if (
        researchData.groq_quick_wins &&
        researchData.groq_quick_wins.length > 0
      ) {
        console.log(
          `📋 ${researchData.groq_quick_wins.length} quick wins saved to: data/quick-wins.csv`,
        );
      }
      ...
      // Debug: Log autoApprove resolution
      console.log(
        `🔍 DEBUG Stage 1: options.autoApprove = ${options.autoApprove}`,
      );
      console.log(
        `🔍 DEBUG Stage 1: this.config.autoApprove = ${this.config.autoApprove}`,
      );
      console.log(`🔍 DEBUG Stage 1: resolved autoApprove = ${autoApprove}`);
      ...
      if (autoApprove) {
        ...
        console.log(
          '   2. Set approval_status = "Yes" for gaps you want to develop',
        );
        console.log(
          "   3. Run Stage 2 to generate topics: node main.js stage topics",
        );
        console.log("");
  ...
  async executeStage2Topics(options = {}) {
    ...
    try {
      ...
      const allGaps = this.csvManager.readCSV(
        this.csvManager.files.researchGaps,
      );
      let approvedGaps = this.csvManager.getApprovedResearchGaps();

      console.log(
        `🐛 DEBUG Stage 2: total gaps in CSV = ${allGaps.length}, approved gaps = ${approvedGaps.length}`,
      );
      if (allGaps.length > 0) {
        const sample = allGaps.slice(0, 5).map((g) => ({
          gap_id: g.gap_id,
          approval_status: g.approval_status,
        }));
        console.log(
          `🐛 DEBUG Stage 2: first 5 gaps approval_status snapshot:`,
      ...
      if (approvedGaps.length === 0) {
        ...
        if (autoApprove) {
          ...
          const approved = this.researcher.autoApproveAll();
          console.log(
            `✅ Auto-approved ${approved} gap(s) for topic generation`,
          );
          approvedGaps = this.csvManager.getApprovedResearchGaps();
          console.log(
            `🐛 DEBUG Stage 2: approved gaps AFTER autoApproveAll = ${approvedGaps.length}`,
          );

          if (approvedGaps.length === 0) {
            // Provide a clearer message: either Stage 1 produced no gaps or approvals didn't persist
            const totalGaps = this.csvManager.readCSV(
              this.csvManager.files.researchGaps,
            ).length;
            console.log(
              `🐛 DEBUG Stage 2: totalGaps after auto-approve = ${totalGaps}`,
            );

            if (totalGaps === 0) {
              throw new Error(
                "No research gaps found. Run Stage 1 (research) before Stage 2 (topics).",
              );
            }
            throw new Error(
              "No approved research gaps found. Auto-approval did not yield any approved gaps.",
            );
          }
        } else {
          ...
          console.log(
            '   2. Change approval_status from "Pending" to "Yes" for desired gaps',
          );
          console.log("   3. Run this stage again: node main.js stage topics");
          console.log("");
          throw new Error(
            "No approved research gaps found. Please approve some gaps first.",
          );
        }
      }
      ...
      console.log(
        `🐛 DEBUG: options.limit = ${options.limit}, this.config.topicLimit = ${this.config.topicLimit}, final limit = ${limit}`,
      );
      ...
      console.log(
        `📝 ${topics.length} topics saved to: data/generated-topics.csv`,
      );
      console.log("");

      // Auto-approve if enabled
      // Use options.autoApprove if provided, otherwise fall back to this.config.autoApprove
      const autoApproveFinal =
        options.autoApprove !== undefined
          ? options.autoApprove
          : this.config.autoApprove;
      if (autoApproveFinal) {
        ...
        console.log(
          '   2. Set approval_status = "Yes" for topics you want to develop',
        );
        console.log(
          "   3. Run Stage 3 for deep research: node main.js stage deep-research",
        );
      }
  ...
  async executeStage3DeepResearch(options = {}) {
    ...
    try {
      ...
      if (customTitle) {
        ...
        console.log(
          "🚀 Bypassing approved topics and running deep research directly on the custom title...",
        );
        ...
        if (!researchResults || researchResults.length === 0) {
          console.log(
            "⚠️  Deep research produced no results for custom title. Check API keys and rerun.",
          );
          return false;
        }
        ...
        if (this.config.autoApprove) {
          ...
          console.log(
            '   2. Approve rows (set approval_status = "Yes") for content creation',
          );
        }

        const topicIds = [
          ...new Set(
            researchResults.map((item) => item.topic_id).filter(Boolean),
          ),
        ];
        ...
        const stageStatus = this.config.autoApprove
        ...
        if (this.config.autoApprove && topicIds.length > 0) {
          const savedResearch =
            this.csvManager.getTopicResearchByTopicIds(topicIds);
          savedResearch.forEach((item) => {
            if (item.topic_research_id) {
              this.csvManager.updateApprovalStatus(
                "topicResearch",
                "topic_research_id",
                item.topic_research_id,
                "Yes",
              );
            }
          });
          console.log(
            `🤖 Auto-approved ${savedResearch.length} research items for content creation`,
          );
        }
      ...
      if (approvedTopics.length === 0) {
        if (this.config.autoApprove) {
          console.log(
            "🤖 Auto-approving high-priority topics for deep research...",
          );
          ...
          if (approvedTopics.length === 0) {
            ...
            if (allTopics.length > 0) {
              console.log(
                "⚠️  Auto-approve fallback: using all topics for deep research because none are marked approved.",
              );
      ...
      if (!researchResults || researchResults.length === 0) {
        console.log(
          "⚠️  Deep research produced no results. Check API keys and rerun.",
        );
        return false;
      }
  ...
  async executeStage4ContentCreation(options = {}) {
    ...
    try {
      ...
      if (customTopic) {
        ...
        const hasCustomTopics = existingTopics.some(
          (t) =>
            t.topic_title &&
            t.topic_title.toLowerCase().includes(customTopic.toLowerCase()),
        );
      ...
      if (approvedResearch.length === 0) {
        ...
        if (approvedResearch.length === 0) {
          ...
          console.log(
            '   • Set approval_status = "Yes" for research ready for content drafting',
          );
          console.log("   • Rerun Stage 4 afterwards");
          throw new Error(
            "No approved deep research available for content creation.",
          );
        }
      }
      ...
      if (limit && approvedResearch.length > limit) {
        ...
        console.log(
          `🔍 Limiting content creation to first ${limit} research item(s)`,
        );
      }
      ...
      const topicIds = [
        ...new Set(createdContent.map((item) => item.topic_id).filter(Boolean)),
      ];
      ...
      const newRecords =
        delta > 0
          ? allContent.slice(-delta)
          : this.csvManager.getContentByTopicIds(topicIds);

      newRecords.forEach((record) => {
        if (
          record.content_id &&
          (record.approval_status === "Pending" ||
            record.approval_status === "Needs-SEO")
        ) {
          this.csvManager.updateContentApprovalStatus(
            record.content_id,
            "Needs-SEO",
          );
        }
  ...
  async executeStage45Validation(options = {}) {
    ...
    try {
      ...
      if (failedCount > 0) {
        console.log(
          "⚠️  Failed content requires fixes before SEO optimization",
        );
        console.log("   • Review validation issues in created-content.csv");
        console.log(
          "   • Auto-correction was attempted using groq/gpt-oss-120b",
        );
  ...
  async executeStage6Publication(options = {}) {
    ...
    try {
      const limit =
        options.limit ??
        this.config.publicationLimit ??
        this.config.contentLimit ??
        this.config.topicLimit ??
        null;
      ...
      published.forEach((item) => {
        ...
        const statusLabel = isPublished
          ...
          : isSimulated
            ...
            : "Publication Failed";
        const approvalState = isPublished
          ? "Published"
          : isSimulated
            ? "Simulated"
            : "Failed";
  ...
  async executeStage7Completion() {
    ...
    if (completionRate >= 80) {
      ...
      console.log(
        "   Current stage: Research and Topics (2/7 stages implemented)",
      );
  ...
  monitorProgress() {
    ...
    const stageStats = {};
    status.forEach((item) => {
      stageStats[item.current_stage] =
        (stageStats[item.current_stage] || 0) + 1;
    });
    ...
    console.log("\n📈 Overall Statistics:");
    console.log(
      `   Research Gaps: ${stats.approvedResearchGaps}/${stats.totalResearchGaps} approved`,
    );
    console.log(
      `   Topics: ${stats.approvedTopics}/${stats.totalTopics} approved`,
    );
  ...
  async syncToGoogleSheets(primarySheet = null) {
    try {
      ...
      if (result.success && result.syncedSheets) {
        console.log(
          `📊 Synced ${result.syncedSheets} sheet(s) to Google Sheets`,
        );
...
if (require.main === module) {
  ...
  const orchestrator = new WorkflowOrchestrator({
    ...
    gapsPerRun:
      parseInt(
        process.argv.find((arg) => arg.startsWith("--gaps="))?.split("=")[1],
      ) || 10,
  });
  ...
  switch (command) {
    ...
    case "stage":
      ...
      if (!stageName) {
        ...
        console.log(
          "  research       - Generate 10 content gap opportunities (append to CSV)",
        );
        console.log(
          "  topics         - Generate strategic topics from approved gaps",
        );
        console.log("  deep-research  - Analyze competitors for each topic");
        console.log("  content        - Create E-E-A-T compliant content");
        console.log(
          "  validation     - Validate content with AI auto-correction",
        );
    ...
    default:
      ...
      console.log(
        "  full                    - Execute complete workflow (all 7 stages)",
      );
      console.log(
        "  stage <stage-name>      - Execute specific workflow stage",
      );
      ...
      console.log("Options:");
      console.log(
        "  --auto-approve          - Auto-approve high-priority items",
      );
      ...
      console.log(
        "  node workflow-orchestrator.js stage topics --auto-approve",
      );
      console.log("  node workflow-orchestrator.js full --gaps=20");
      console.log("");
      console.log("Available stages:");
      console.log(
        "  research, topics, deep-research, content, validation, seo, publication, completion",
      );
      console.log("");
      console.log(
        '💡 Tip: Run "research" stage multiple times to accumulate gaps!',
      );
  }
}
