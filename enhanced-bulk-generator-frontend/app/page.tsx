...
export default function Home() {
  ...
  const getSeoTitle = (seoMetadata: any): string => {
    if (!seoMetadata) return "N/A";

    if (typeof seoMetadata === "string") {
      try {
        const parsed = JSON.parse(seoMetadata);
        return parsed?.title || "N/A";
      } catch {
        return "N/A";
      }
    }

    if (typeof seoMetadata === "object") {
      return (seoMetadata as any).title || "N/A";
    }
  ...
  const executeStage = async (stageId: number) => {
    ...
    try {
      ...
      try {
        ...
        addLog(
          `⚠️  Warning: Could not auto-approve: ${approveError instanceof Error ? approveError.message : "Unknown error"}`,
        );
      ...
      if (reader) {
        ...
        while (true) {
          ...
          for (const segment of segments) {
            ...
            // Each SSE event may have multiple lines; process all data: lines
                              ...
                              <button
                                onClick={async () => {
                                  try {
                                    ...
                                    console.log(
                                      `📥 Downloading markdown for content_id: ${contentRow.content_id}, title: ${getSeoTitle(contentRow.seo_metadata)}`,
                                    );
                                    const response = await fetch(
                              ...
                              <button
                                onClick={async () => {
                                  try {
                                    ...
                                    console.log(
                                      `📥 Downloading HTML for content_id: ${contentRow.content_id}, title: ${getSeoTitle(contentRow.seo_metadata)}`,
                                    );
                                    const response = await fetch(
                              ...
                              <button
                                onClick={async () => {
                                  try {
                                    ...
                                    console.log(
                                      `📥 Downloading raw content for content_id: ${contentRow.content_id}, topic_id: ${contentRow.topic_id || "N/A"}`,
                                    );
                                    const response = await fetch(
                        ...
                        <div className="max-h-96 overflow-y-auto">
                          {stageData[stage.id].data.map(
                            (row: any, idx: number) => (
                              <div
                                key={idx}
                                className="mb-3 p-3 bg-gray-50 rounded border border-gray-200 text-xs relative"
                              >
                                {/* Edit Button */}
                                <button
                                  onClick={() =>
